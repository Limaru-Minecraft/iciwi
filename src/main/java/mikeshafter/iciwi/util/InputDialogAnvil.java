...
public static class Button {
	...
	/**
	 ...
	 @param material item
	 */
	public void setItemMaterial (ItemStack material) {setMaterial(material == null ? null : material.getType());}
...
private class EventListenerBase implements Listener {
	...
	protected boolean mustHandle (InventoryEvent event) {
		...
		return !(event.getInventory() instanceof AnvilInventory) || !_openInventories.contains(event.getView());// yay!
	}

	@SuppressWarnings ("deprecation") @EventHandler (priority = EventPriority.LOWEST) public void onInventoryClick (InventoryClickEvent event) {
		if (mustHandle(event)) return;
...
private class EventListenerFull extends EventListenerBase {
	@EventHandler (priority = EventPriority.LOWEST) public void onPrepareAnvil (PrepareAnvilEvent event) {
		if (mustHandle(event)) return;
