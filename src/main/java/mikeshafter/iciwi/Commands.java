...
@Command("iciwi fares deleteclass <fareclass>")
@CommandDescription("Removes a station and all its associated fares from the data.")
@Permission("iciwi.fares.deletestation")
public void delete_class (
	final @NonNull CommandSender sender,
	final Iciwi plugin,
	final @NonNull @Argument(value = "fareclass", suggestions = "") String fareclass
) {
	fares.deleteClass(fareclass);
	sender.sendMessage(formatString("All fares to all stations from %s has been deleted.", fareclass));
}
