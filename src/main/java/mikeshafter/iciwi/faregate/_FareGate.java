...
//         flags += 1; // Validator
//       if (args.contains("S"))
//         flags += 2; // Sideways sign
//       if (args.contains("L"))
//         flags += 4; // Lefty sign
//       if (args.contains("D"))
//         flags += 8; // Double fare gate
//       if (args.contains("R"))
//         flags += 16; // Redstone activator
//       if (args.contains("E"))
//         flags += 32; // Eye-level sign.
//
//       if ((flags & 1) == 1) {
...
//       // LM-style fare gates
//       // location matters
//       if ((gateType == GateType.ENTRY || gateType == GateType.EXIT || gateType == GateType.MEMBER)
//           && (signLoc != null && signLoc.getBlock().getState() instanceof Sign
//               && signLoc.getBlock().getState().getBlockData() instanceof WallSign sign)) {
//
...
//           switch (signFacing) {
//             case SOUTH -> this.faceMap.put(signLoc.clone().add(locVector[0], locVector[1], locVector[2]),
//                 parseOpenDirection(signFacing, locVector[3]));
//             case NORTH -> this.faceMap.put(signLoc.clone().add(-locVector[0], locVector[1], -locVector[2]),
//                 parseOpenDirection(signFacing, locVector[3]));
//             case EAST -> this.faceMap.put(signLoc.clone().add(locVector[2], locVector[1], -locVector[0]),
//                 parseOpenDirection(signFacing, locVector[3]));
//             case WEST -> this.faceMap.put(signLoc.clone().add(-locVector[2], locVector[1], locVector[0]),
//                 parseOpenDirection(signFacing, locVector[3]));
...
//           && signLoc.getBlock().getBlockData() instanceof org.bukkit.block.data.type.Sign sign) {
//
...
//       if (location.getBlock().getBlockData() instanceof Openable || location.getBlock() instanceof Powerable
//           || direction == null) {
//         // no animation
...
//
//   public void hold() {
//     plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, this::close,
//         plugin.getConfig().getLong("gate-close-delay"));
//   }
//
...
//     });
//
//     if (fareGateBlocks.size() > 0)
//       fareGateBlocks.forEach(FareGateBlock::closeGate);
//   }
// }
