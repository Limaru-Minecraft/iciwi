...
public class CardSql{
  ...
  public void initTables(String[] discounts) {
    ...
    sql.add("CREATE TABLE IF NOT EXISTS cards (serial_prefix text ,serial integer ,value real, PRIMARY KEY (serial_prefix, serial)); " +
            "CREATE TABLE IF NOT EXISTS log (serial_prefix text,serial integer,start_station text,end_station text,price real,FOREIGN KEY (serial_prefix) references cards(serial_prefix) FOREIGN KEY (serial) references cards(serial), PRIMARY KEY (serial_prefix, serial)");
  ...
  public void log(String serial_prefix, int serial, String start_station, String end_station, double price) {
    String sql = "INSERT INTO cards(serial_prefix,serial,start_station, end_station, price) VALUES(?, ?, ?, ?, ?)";
  ...
  public void setDiscount(String serial_prefix, int serial, String operator, long expiry) {
    String sql = expiry > 0 ? "INSERT INTO ?(serial_prefix,serial,start_station, end_station, price) VALUES(?, ?, ?)" : "DELETE FROM ? where serial_prefix = ? and serial = ?";
    try (Connection conn = this.connect(); PreparedStatement statement = conn.prepareStatement(sql)) {
